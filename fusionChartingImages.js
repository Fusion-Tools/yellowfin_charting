define(function () {
    ...
    const getRgbListCssStyle = function (rgbList) {
        /*
        Get the CSS style string for a given color
        this can be applied to a <img> tag to change its color
    
        rgbList: array of 3 numbers between 0 and 255
        ...
        */
        ...
        const color = new Color(rgbList[0], rgbList[1], rgbList[2]);
    ...
    function getCssStyleForColor(color) {
        /*
        Converts a color to a CSS filter string.
    
        string: color - The color to convert. Cna be in the forms:
            - #RRGGBB
            - #RGB
            - rgb(r, g, b)
            - rgba(h, s, l)
    
        returns: string - The corresponding CSS filter string.
        */

        const plotlyAxisImages = [];

        var rgbList = [255, 255, 255];
        ...
        function RgbStringToList(hex) {
            // format: rgb(255, 255, 255) into a list of [255, 255, 255]

            const rgbString = hex.replace("rgb(", "").replace(")", "");
            const rgbList = rgbString.split(",").map((x) => parseInt(x));

            return rgbList;
        }

        function RgbaStringToList(hex) {
            // format: rgba(255, 255, 255, 255) into a list of [255, 255, 255]

            const rgbString = hex.replace("rgba(", "").replace(")", "");
            const rgbList = rgbString.split(",").map((x) => parseInt(x));
        ...
        if (color.includes("rgb(")) {
            rgbList = RgbStringToList(color);
        } else if (color.includes("rgba(")) {
            rgbList = RgbaStringToList(color);
        } else {
            rgbList = hexToRgb(color);
        }

        colorStyle = getRgbListCssStyle(rgbList)
